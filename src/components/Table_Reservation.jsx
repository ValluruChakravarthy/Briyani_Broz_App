...
// Handling this because Supabase is formatted in en-CA
...
const imagecard = [
  "https://assets.cntraveller.in/photos/68931f2345b24142f6811ac5/16:9/w_2560%2Cc_limit/Bungalow%2520%25C2%25A9%2520Katrine%2520Moite_3P4A9726-HDR.jpg",
...
function TableReservation() {
  ...
  async function getslots(date) {
    ...
    try {
      ...
      setBookedSlots(
        data.map((booking) => booking.TimeSlotChosen)
      );

      // Get your booking slots using user_id
  ...
  if (!slotSelected) {
    ...
    // Read the reservation form data
  const formData = new FormData(form);
    ...
    try {
      ...
      setConfirmedMessage(
        `Table booked for ${booking.DateChosen} at ${booking.TimeSlotChosen}!`
      );

      setShowConfirmed(true);

    await getslots(selectedDate); 
  ...
  return (
    <div>
      ...
      {/* Slide photo sections */}
      <section className="relative top-18">

        <img
          className='w-full h-[52vw] overflow-clip'
          src={imagecard[currentImage]}
        />
      ...
      <section className='max-w-7xl mx-auto py-40 px-8 grid grid-cols-1 md:grid-cols-2 gap-10'>
        ...
        <div className='flex flex-col gap-6 items-center border-2 p-6 md:rounded-[20%]'>
          ...
          <div>
            ...
            {/* Time slots */}
            <div className='gap-4 grid grid-cols-3 text-md'>
        ...
        >
          ...
          <p className='text-center mx-auto font-extrabold text-slate-800'>
            Reserve your slot
          </p>
